...
const BetterPartiesList: React.FC<Props> = ({ userId }) => {
  ...
  const { data, loading, error, fetchMore } = usePaginatePartiesQuery({
  ...
  return (
    <PartiesListContext.Provider value={contextState}>
      <PartiesListWrapper>
        ...
        <PartiesListCardGrid
          parties={
            data && data.partiesConnection ? data.partiesConnection.edges : []
          }
...
export default BetterPartiesList;
