...
export function useAudio(audioRef: React.RefObject<HTMLAudioElement>, disabled: boolean): UseAudioApi {
    ...
    const skip: UseAudioApi['skip'] = React.useCallback((amountToSkip, shouldPauseOnSkip = false) => {
        ...
        else if (timeAfterSkipping <= 0)
            actualAmountToSkip = 0;
        else actualAmountToSkip = timeAfterSkipping;
        setTime(actualAmountToSkip, shouldPauseOnSkip);
    }, [state.audioCurrentTime, state.audioDuration])

    const toggle = React.useCallback(() => state.playing ? pause() : play(), [state.playing])
    ...
    function setTime(timeToSet: number, pauseOnSet = false) {
        if (!audioRef.current) return;
        pauseOnSet && audioRef.current.pause();
        audioRef.current.currentTime = timeToSet;
        setState(prevState =>
            ({ ...prevState, audioCurrentTime: timeToSet, playing: !pauseOnSet || timeToSet == prevState.audioDuration }))
    }
